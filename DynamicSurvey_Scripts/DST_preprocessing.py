...
def cleanRigData(rig_file_path):
    ...
    col_to_drop = ["RT_TVD", "RT_ACTIVITY", "RT_Botm", "RT_SLIDE", "RT_ROP", "RT_RPM.W", 'RT_SLIDE_BIT',
               "MR_INC.M", "MR_LINC.M", "MR_AZM.M", "MR_LAZM.MR", "MR_DIPA.M", "MR_MAGF.M", "MR_TEMP.M", "MR_VIBT.MR", "MR_GAMA.5S5A", "MR_ROTATION", "MR_FLOW.M",
               "PD_Temperature", "PD_X Vibration", "PD_Y Vibration", "PD_Gamma", "PD_Pump Event"]
    ...
    df['GPM'] = (df['RT_PUMP_ON'] + df['RT_PUMP_OFF']) / 2
    col_to_drop.append('RT_PUMP_ON')
    col_to_drop.append('RT_PUMP_OFF')
    # delete irrelevant columns in well df
    col_to_drop += [*new_col_names.values()]
    df.drop(columns=col_to_drop, inplace=True)
    # get rid of rows with no data in columns (except time)
    df.loc[:, df.columns != "Time"].dropna(axis=0, inplace=True, how = 'all')
